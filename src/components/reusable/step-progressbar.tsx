...
export const StepProgressBar = () => {
  const { accessToken, data } = useSelector(
  ...
  return (
    <div className="flex items-center gap-6 py-4">
      <div
        className={`flex items-center gap-3 ${data && accessToken ? "text-green-500" : ""}`}
      >
        <CircleCheckBig size={20} />
        <h1 className="text-sm font-medium">EMAIL & PASSWORD</h1>
      </div>
      <div
        className={`w-20 border-b-2 ${accessToken && data ? "text-green-500" : ""}`}
      ></div>
